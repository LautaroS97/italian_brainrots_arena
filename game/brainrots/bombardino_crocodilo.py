...
def _ventarron():
    wk = weaken_next_attack(0.5)   # –50 % daño
    ec = extra_energy_cost(1.25)   # +25 % PP
    def fn(att, riv):
        res1 = wk(att, riv)
        res2 = ec(att, riv)
    ...
    return Brainrot(
        ...
        skills=[
            Skill(
                ...
                animation={"file_root": f"{base}/masticada", "fps": 8, "movement": True, "collision": True}
            ),
            Skill(
                name="Ventarrón",
                ...
                animation={"file_root": f"{base}/ventarron", "fps": 6, "freeze": True}
            ),
            Skill(
                name="Bombazo",
                ...
                animation={"file_root": f"{base}/bombazo", "fps": 6, "movement": True, "collision": True}
            ),
            Skill(
                name="Crocomisil",
                ...
                is_direct_attack=False,
                animation={"file_root": f"{base}/crocomisil", "fps": 7, "movement": True, "collision": True}
            )
        ]
    )