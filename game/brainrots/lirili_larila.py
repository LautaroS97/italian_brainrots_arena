...
def _arenas_movedizas():
    wk = weaken_next_attack(0.5)   # Debilitado 50 %
    ec = extra_energy_cost(1.25)  # PP +25 %
    def fn(att, riv):
        res1 = wk(att, riv)
        res2 = ec(att, riv)
    ...
    return Brainrot(
        ...
        skills=[
            Skill(
                ...
                animation={"file_root": f"{base}/patadon", "fps": 8, "movement": True, "collision": True}
            ),
            Skill(
                ...
                animation={"file_root": f"{base}/arenas_movedizas", "fps": 6, "freeze": True}
            ),
            Skill(
                ...
                animation={"file_root": f"{base}/bomba_de_espinas", "fps": 7, "movement": True, "collision": True}
            ),
            Skill(
                ...
                is_direct_attack=False,
                animation={"file_root": f"{base}/volver_al_pasado", "fps": 6}
            )
        ]
    )